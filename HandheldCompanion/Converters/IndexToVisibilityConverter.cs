...
public sealed class IndexToVisibilityConverter : IValueConverter
{
    ...
    {
        var sValue = System.Convert.ToString(value);
        var sParameter = System.Convert.ToString(parameter);

        if (!string.IsNullOrEmpty(sParameter))
        {
            var parameters = sParameter.Split(new[] { '|' });
            foreach (var idx in parameters)
