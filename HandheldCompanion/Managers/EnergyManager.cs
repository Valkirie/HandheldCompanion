...
public static class EnergyManager
{
    ...
    public enum EfficiencyMode
    {
        ...
        /// <summary>
        ///     Description: Applications that explicitly tag processes with SetProcessInformation or threads with
        ///     SetThreadInformation. <br />
        ...
        /// <summary>
        ///     Description: Threads explicitly tagged by the Multimedia Class Scheduler Service to denote multimedia batch
        ///     buffering. <br />
        ...
        /// <summary>
        ///     Description: Threads explicitly tagged by Multimedia Class Scheduler Service to denote that audio threads require
        ///     performance to meet deadlines. <br />
    ...
    public static void ToggleEfficiencyModes()
    {
        ...
        Parallel.ForEach(ProcessManager.GetProcesses(),
    ...
    public static void RestoreEfficiencyModes()
    {
        Parallel.ForEach(ProcessManager.GetProcesses(),
            ...
            process => { ToggleEfficiencyMode(process, EfficiencyMode.High); });
    }
    ...
    public static void ToggleEfficiencyMode(int pId, EfficiencyMode mode, ProcessEx parent = null)
    {
        ...
        {
            var hProcess =
                OpenProcess((uint)(ProcessAccessFlags.QueryLimitedInformation | ProcessAccessFlags.SetInformation),
                    false, (uint)pId);
        ...
        var processEx = ProcessManager.GetProcess(pId);
        if (processEx is null)
            return;

        // update efficiency mode (visual)
        processEx.SetEfficiencyMode(mode);
        LogManager.LogDebug("Process {0} and {1} subprocess(es) have efficiency mode set to: {2}", processEx.Title,
            processEx.Children.Count, mode);

        // apply efficiency mode to child processes
        processEx.RefreshChildProcesses();
        foreach (var childId in processEx.Children)
    ...
    public static bool GetProcessInfo(IntPtr handle, PROCESS_INFORMATION_CLASS piClass, out object processInfo)
    {
        ...
        {
            var sizeOfProcessInfo = Marshal.SizeOf(infoType);
