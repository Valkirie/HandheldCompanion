...
public class IMUInclinometer : IMUSensor
{
    ...
    public IMUInclinometer(SensorFamily sensorFamily, int updateInterval)
    {
    ...
    public Vector3 GetCurrentReading(bool center = false)
    {
        ...
        var angle_x_psi = -1 * Math.Atan(reading.Y / Math.Sqrt(Math.Pow(reading.X, 2) + Math.Pow(reading.Z, 2))) * 180 /
                          Math.PI;
        var angle_y_theta = -1 * Math.Atan(reading.X / Math.Sqrt(Math.Pow(reading.Y, 2) + Math.Pow(reading.Z, 2))) *
            180 / Math.PI;
